...
class AboutControlStatements < EdgeCase::Koan
  ...
  def test_if_statements_return_values
    ...
    value = if false#false is never true.
              :true_value
            else
              :false_value
            end
  ...
  def test_if_statements_with_no_else_with_false_condition_return_value
    ...
    assert_equal nil, value#nil is empty (kinda)
  end
  ...
  def test_break_statement
    ...
    result = 1
    while true
      break unless i <= 10#only break if i >10
      result = result * i
      i += 1
    end
    assert_equal 3628800, result
  ...
  def test_next_statement
    ...
    while i < 10
      i += 1
      next if (i % 2) == 0# the next skips i when divisible by 2
      result << i
    end
